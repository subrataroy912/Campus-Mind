import { MenuIcon } from "lucide-react";
import { BsThreeDots } from "react-icons/bs";
import { CgClose } from "react-icons/cg";
import avatar from "../../assets/avatar.png"
function NotificationCard({ onClose }) {
  return (
    <div
      className='absolute flex flex-col items-center z-10 top-0 right-0 w-full h-screen bg-surface md:right-10 md:top-15 md:w-[50%] md:min-w-80 md:h-[calc(100vh-80px)] md:shadow-2xl md:shadow-text-heading rounded-lg'>
      <div className="flex items-center justify-between p-4 bg-canvas w-full h-4">
        <h2 className="font-serif">Notifications</h2>
        <CgClose className="bg-text-muted rounded-lg text-xl cursor-pointer" onClick={onClose} />
      </div>
      <div className="flex flex-col items-center w-full overflow-y-auto">
        <NotificationCardItems avatar={avatar} author={"Nabojeet BiswasNabojeet BiswasNabojeet BiswasNabojeet BiswasNabojeet BiswasNabojeet BiswasNabojeet BiswasNabojeet BiswasNabojeet BiswasNabojeet Biswas"} createdAt={"8-28-2026"} timestamp={"8:10 PM"} content={"Lorem ipsum dolor sit amet consectetur adipisicing elit. Quos assumenda dignissimos nostrum, ex adipisci quidem!"} />
        <NotificationCardItems avatar={avatar} author={"Subho Free Fire"} createdAt={"8-28-2026"} timestamp={"8:10 PM"} content={"Lorem ipsum dolor sit amet consectetur adipisicing elit. Quos assumenda dignissimos nostrum, ex adipisci quidem!"} />
        <NotificationCardItems avatar={avatar} author={"Dhrubo _ _ _"} createdAt={"8-28-2026"} timestamp={"8:10 PM"} content={"Lorem ipsum dolor sit amet consectetur adipisicing elit. Quos assumenda dignissimos nostrum, ex adipisci quidem!"} />
        <NotificationCardItems avatar={avatar} author={"Sayil"} createdAt={"8-28-2026"} timestamp={"8:10 PM"} content={"Lorem ipsum dolor sit amet consectetur adipisicing elit. Quos assumenda dignissimos nostrum, ex adipisci quidem!"} />
        <NotificationCardItems avatar={avatar} author={"Sayil"} createdAt={"8-28-2026"} timestamp={"8:10 PM"} content={"Lorem ipsum dolor sit amet consectetur adipisicing elit. Quos assumenda dignissimos nostrum, ex adipisci quidem!"} />
        <NotificationCardItems avatar={avatar} author={"Sayil"} createdAt={"8-28-2026"} timestamp={"8:10 PM"} content={"Lorem ipsum dolor sit amet consectetur adipisicing elit. Quos assumenda dignissimos nostrum, ex adipisci quidem!"} />
        <NotificationCardItems avatar={avatar} author={"Sayil"} createdAt={"8-28-2026"} timestamp={"8:10 PM"} content={"Lorem ipsum dolor sit amet consectetur adipisicing elit. Quos assumenda dignissimos nostrum, ex adipisci quidem!"} />
        <NotificationCardItems avatar={avatar} author={"Sayil"} createdAt={"8-28-2026"} timestamp={"8:10 PM"} content={"Lorem ipsum dolor sit amet consectetur adipisicing elit. Quos assumenda dignissimos nostrum, ex adipisci quidem!"} />
        <NotificationCardItems avatar={avatar} author={"Sayil"} createdAt={"8-28-2026"} timestamp={"8:10 PM"} content={"Lorem ipsum dolor sit amet consectetur adipisicing elit. Quos assumenda dignissimos nostrum, ex adipisci quidem!"} />
      </div>
    </div>
  )
}

export default NotificationCard;

function NotificationCardItems({
  author,
  avatar,
  createdAt,
  timestamp,
  content,
}) {
  return (
    <div className="w-full flex justify-between items-start gap-4 p-4 border-b border-border last:border-b-0 hover:bg-canvas transition-colors">

      {/* Avatar */}
      <div className="shrink-0 rounded-full shadow-sm shadow-text-muted">
        <img
          src={avatar}
          alt={`${author}'s avatar`}
          className="w-7.5 h-7.5 rounded-full object-cover"
        />
      </div>

      {/* Notification Content */}
      <div className="flex flex-col flex-1 min-w-0">
        <h4 className="text-sm font-semibold text-text-heading tracking-tight truncate">
          {author}
        </h4>

        <div className="flex items-center space-x-2 mt-0.5">
          <span className="text-xs text-text-muted">{createdAt}</span>
          <span className="text-xs text-text-muted">•</span>
          <span className="text-xs text-text-muted">{timestamp}</span>
        </div>

        <p className="mt-1.5 text-sm text-text-main leading-relaxed tracking-tight line-clamp-2">
          {content}
        </p>
      </div>

      {/* Actions */}
      <div className="shrink-0">
        <button
          type="button"
          aria-label="Notification options"
          className="p-1 rounded-full text-text-muted hover:text-text-main hover:bg-canvas transition-colors"
        >
          <BsThreeDots className="text-lg" />
        </button>
      </div>
    </div>
  );
}
