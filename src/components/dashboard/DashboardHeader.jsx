...
export default function DashboardHeader({ isSidebarOpen, onSidebarToggle }) {
  ...
  return (
    <header className="flex min-h-16 items-center justify-between border-b border-border bg-surface px-3 sm:px-6">
      <button
      ...
      <p className="hidden text-sm text-text-muted md:block">
        A thoughtful place to learn together.
      </p>
      ...
      {menuOpen && (
        <nav className="absolute left-0 top-16 z-30 w-72 border-b border-r border-border bg-surface p-3 shadow-lg md:hidden">
          ...
          <div className="mt-3 space-y-1 border-t border-border pt-3">
            <Link
              ...
              onClick={() => setMenuOpen(false)}
              className="flex items-center gap-3 rounded-md p-3 text-sm font-medium text-text-main hover:bg-canvas"
            >
              <Ticket size={20} />
              Join with code
            </Link>
            <Link
              ...
              onClick={() => setMenuOpen(false)}
              className="flex items-center gap-3 rounded-md bg-primary p-3 text-sm font-medium text-surface hover:bg-primary-hover"
            >
              <Plus size={20} />
              Create a class
            </Link>
          ...
          <Link
            ...
            onClick={() => setMenuOpen(false)}
            className="mt-3 flex items-center gap-3 rounded-lg p-3 text-sm font-medium text-text-main hover:bg-canvas"
          >
            <Settings size={20} />
            Settings
          </Link>
