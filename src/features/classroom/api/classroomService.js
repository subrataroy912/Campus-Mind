...
export const createClassroom = async (details) => { const classroom = { ...details, id: `class-${Date.now()}`, title: details.className, subtitle: details.section || 'New section', code: `${details.className.slice(0, 4).toUpperCase().padEnd(4, 'X')}-${Math.random().toString(36).slice(2, 6).toUpperCase()}`, teacher: { name: 'You', avatar: null, initials: 'YO' }, memberCount: 1, onlineCount: 1, unreadCount: 0, deadline: null }; const classrooms = [...read(), classroom]; write(classrooms); return delay(classroom) }
