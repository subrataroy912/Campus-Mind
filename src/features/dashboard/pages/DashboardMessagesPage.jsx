    ...
    >
      ...
      <span className="min-w-0 flex-1">
        <span className="flex items-center justify-between gap-2">
          ...
          <span className="shrink-0 text-xs text-text-muted">{conversation.time}</span>
        </span>
        <span className="mt-0.5 block truncate text-xs text-text-muted">
          {conversation.classroom}
        </span>
        <span className="mt-0.5 flex items-center justify-between gap-2">
          <span className="truncate text-sm text-text-muted">{conversation.lastMessage}</span>
    ...
    <div className="flex h-full flex-col">
      <div className="flex items-center gap-3 border-b border-border px-4 py-3">
        ...
        <div className="min-w-0">
          <p className="truncate text-sm font-semibold text-text-heading">{conversation.name}</p>
          <p className="truncate text-xs text-text-muted">{conversation.classroom}</p>
        </div>
      </div>
            ...
            >
              ...
              <span
                className={`mt-1 block text-[10px] ${
                  message.from === "me" ? "text-primary-foreground/70" : "text-text-muted"
                }`}
              >
                {message.time}
              </span>
...
export default function DashboardMessagesPage() {
      ...
      <div className="mx-auto max-w-4xl p-3 sm:p-6">
        <header>
          ...
          <p className="mt-2 text-text-muted">
            Messages are kept with the classes they belong to, so it is easier to find the context later.
          </p>
        </header>
        <div className="mt-8">
          <EmptyState
  ...
  return (
    <div className="mx-auto flex h-[calc(100dvh-4rem)] max-w-6xl flex-col p-3 sm:p-6">
      ...
      <div className="mt-6 grid min-h-0 flex-1 grid-cols-1 overflow-hidden rounded-2xl bg-surface shadow-sm ring-1 ring-border lg:grid-cols-[320px_1fr]">
        <div
        ...
        <div className={`min-h-0 ${activeConversation ? "flex" : "hidden lg:flex"}`}>
          {activeConversation ? (
            <ChatThread conversation={activeConversation} onBack={() => setActiveId(null)} />
          ) : (
