...
import { useExploreData } from "@/features/explore/hooks/useExploreData.js";
...
export default function ProfilePage() {
  ...
  const { users = [] } = useExploreData(currentUser?.id);
  const isOwner = (!userId || userId === currentUser?.id) && !preview;
  const viewedUser = userId && userId !== currentUser?.id ? users.find((item) => item.id === userId) : currentUser;
  ...
  const classes = isOwner ? classrooms.filter((item) => currentClassIds.includes(item.id)) : classrooms.filter((item) => sharedIds.includes(item.id));
