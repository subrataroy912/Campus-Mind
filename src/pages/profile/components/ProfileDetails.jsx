  ...
  return <dl className="grid gap-4 sm:grid-cols-3">
    {details.map(({ label, value, icon }) => {
      ...
      return <div key={label} className="flex gap-3">
        ...
        <div><dt className="text-xs font-semibold uppercase tracking-wide text-text-muted">{label}</dt><dd className="mt-1 text-sm font-medium text-text-main">{value}</dd></div>
      </div>
    })}
  </dl>
}
